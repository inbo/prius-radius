# OPMERKING
# code aangereikt door Diederik om PrIUS-2-conforme kaartjes te genereren op basis van de output van WiSDM (aangereikt  door Soria)
# enkel voor archiefdoeleinden; de WiSDM-output is niet overgebracht naar, en onderstaande code is niet aangepast aan, de PrIUS-2-repo
...
# .../PrIUS2/output/distribution_maps/Climate_only
out_dir <- file.path(
  prius2_dir,
  "output", "distribution_maps", "Climate_only"
)
...
# --- find tifs ONLY under Climate/Current/Predictions/Rasters ---
tif_files <- unlist(lapply(species_dirs, function(sp_dir) {
  
  climate_raster_dir <- file.path(
    sp_dir,
    "Climate", "Current", "Predictions", "Rasters"
  )
  ...
  list.files(
    climate_raster_dir,
    pattern = "_Climate_current_ensemble\\.tif$",
    full.names = TRUE,
    recursive = FALSE
  )
}), use.names = FALSE)
...
if (length(tif_files) == 0) {
  stop("No files found matching '*_Climate_current_ensemble.tif' in Climate/Current/Predictions/Rasters.")
}

message("Found ", length(tif_files), " climate tif(s).")

# --- loop ---
for (tif_path in tif_files) {
  
  base_name <- file_path_sans_ext(basename(tif_path))
  GBIF_canonicalName <- sub("_Climate_current_ensemble$", "", base_name)
...
message("All done (Climate-only). Output: ", out_dir)
