...
occs <- occ_flanders %>%
  filter(Soort %in% c("Gevlekteamerikaanse Rivierkreeft", "Rodeamerikaanse Rivierkreeft", "Californischerivierkreeft"))
...
map <- ggplot() +
  ...
  scale_fill_manual(breaks = names(colors[grouping_factors]), values = unname(colors[grouping_factors])) +
  geom_sf(data = occs, aes(color = Soort)) +
  theme_void() +
  scale_color_discrete(labels = c("Californische rivierkreeft", "Gevlekte Amerikaanse rivierkreeft", "Rode Amerikaanse rivierkreeft")) +
  theme(plot.margin=grid::unit(c(0,0,0,0), "mm"),
        legend.title = element_blank(),
