...
## Taxa
```{r define_taxa}
taxonKeys <- read_csv("./radius/data/input/radius_species_list.csv") %>%
  pull("GBIF_code") 
```
...
gbif_downloadKey <- "0012987-231120084113126"
```
...
occ_flanders <- occ_flanders %>%
  select(year, month, day, decimalLongitude, decimalLatitude, speciesKey, coordinateUncertaintyInMeters) %>%  #following be_species_cube column formatting, can be changed if needed
...
write.csv(in_flanders, "./radius/data/intermediate/in_flanders.csv", row.names = FALSE)
write.csv(not_in_flanders, "./radius/data/intermediate/not_in_flanders.csv", row.names = FALSE)
```
