...
species_list <- read_csv("./radius/data/input/radius_species_list.csv") %>%
  mutate(buf_dist = 100) # Manually set buffer distance
```
...
occ_flanders <- read_csv("./radius/data/input/gbif_occ_flanders.csv") %>%
  ...
  select(year, decimalLongitude, decimalLatitude, speciesKey, Soort, coordinateUncertaintyInMeters, buf_dist) %>%
...
for (soort in unique(occ_flanders$Soort)) {
  ...
  occ_species_buf <- occ_species %>%
    st_buffer(dist = unique(occ_species$buf_dist)) %>%
    st_union() %>% 
    st_cast('POLYGON') %>% 
    st_sf() %>%
  ...
  list.results[[7]] <- rbind(list.results[[7]], data.frame(
    Soort = soort, 
    ofVL = as.character(round(as.numeric(total_area / Vlaanderen_grenzen$OPPERVL),5))
  ))
