...
leaflet() %>%
  addTiles(data = "OSM") %>%
  addPolygons(data = Vlaanderen_grenzen, fillOpacity = 0, weight = 2, color = "black") %>%
  addPolylines(data = VHAWaterlopen %>% st_transform(4326), fillOpacity = 1, weight = 2, color = "blue") #%>%
  #addPolylines(data = lu_sbp_pls_buffered %>% st_transform(4326), color = "blue", opacity = 1, weight = 2)

```

# Watersystemen Vlaanderen
```{r}
## Bekken
...
## Oppervlaktewaterlichamen: afstroomzone van Vlaams oppervlaktewaterlichaam (A0 afstroomzone)
request <- ("https://geo.api.vlaanderen.be/Oppervlaktewaterlichamen/wfs?service=WFS&version=2.0.0&request=GetFeature&typeName=Oppervlaktewaterlichamen:AfstrZonA0&outputFormat=application/json&maxFeatures=1&srsName=epsg:3857")

AfstrZonA0 <- read_sf(request) %>%
  st_set_crs(3857) %>%
  st_transform(31370) %>%  
  ensure_multipolygons() 
...
request <- ("https://geo.api.vlaanderen.be/VHAWaterlopen/wfs?service=WFS&version=2.0.0&request=GetFeature&typeName=VHAWaterlopen:VHAG&outputFormat=application/json&maxFeatures=1&srsName=epsg:3857")

VHAWaterlopen <- read_sf(request) %>%
  st_set_crs(3857) %>%
  st_transform(31370) %>%
  ensure_multilinestrings()
...
ggplot() + 
  geom_sf(data = Provincies_grenzen) +
  geom_sf(data = AfstrZonA0, color = "blue") +
  geom_sf(data = lu_sbp_pls, fill = "red", color = "red") +
  geom_sf(data = lu_sbp_pls_buffered, fill = "red", color = "red")

overlap_sbp_pls <- AfstrZonA0[st_intersection(AfstrZonA0, lu_sbp_pls, sparse = FALSE), ]

x <- st_intersection(lu_sbp_pls, AfstrZonA0)

ggplot() + 
  geom_sf(data = Provincies_grenzen) +
  geom_sf(data = AfstrZonA0, color = "blue") +
  geom_sf(data = overlap_sbp_pls, color = "purple", fill = "purple") +
  geom_sf(data = lu_sbp_pls, fill = "red", color = "red") +
  geom_sf(data = lu_sbp_pls_buffered, fill = "red", color = "red")

ggplot() + 
  geom_sf(data = Provincies_grenzen) +
  geom_sf(data = AfstrZonA0, color = "purple") 

leaflet() %>%
  addTiles(data = "OSM") %>%
  addPolygons(data = AfstrZonA0 %>% st_transform(4326), fillOpacity = 0, color = "purple", weight = 2, label = ~id) %>%
  addPolygons(data = cluster_sbp_vissen_wgs84, fillColor = "purple", weight = 2, label = ~A0_naam_2) 
...
# Calculating occupancy metrics SBP beekvissen (sbp_pls)
```{r}
sbp_pls_species <- c("Amerikaanse stierkikker",
                     "blauwbandgrondel", "Kesslergrondel", "Pontische stroomgrondel", "marmergrondel", "zwartbekgrondel", "zonnebaars",
                     "Californische rivierkreeft", "gestreepte Amerikaanse rivierkreeft", "gevlekte Amerikaanse rivierkreeft", 
                     "rode Amerikaanse rivierkreeft", "Turkse rivierkreeft",
                     "Chinese wolhandkrab"
)
...
for (i in sbp_pls_species) {
  ...
  for (j in seq_along(list.dfs[1:3])) {
...
SBP_pls_extra <- rbind(list.results$SBP_pls, list.results$ws_bekken, list.results$ws_deelbek)
```
