...
for (soort in unique(occ_flanders$Soort)) {
  ...
  for (i in seq_along(list.dfs[1:5])) {
    ...
    gebied_summary <- gebied %>%
      st_drop_geometry() %>%
      select(code = if (i == 1) "gebcode" else if (i == 2) "na2000code" else if (i == 3) "type" else if (i == 4) "regio_abb" else "eigendomtype_abb", geb_area) %>%
      group_by(code) %>%
    ...
    if (nrow(occ_species_intersection) != 0) {  # skip following steps if there's no overlap 
      ...
      if (i %in% c(1, 2, 3)) {
        ...
        overlap_per_gebied <- occ_species_intersection %>%
          group_by(code = if (i == 2) na2000code else if (i == 3) type else gebcode) %>%
