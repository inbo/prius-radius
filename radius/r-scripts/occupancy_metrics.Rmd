...
for (i in unique(occ_flanders$Soort)) {
  ...
  list.results[[7]] <- rbind(list.results[[7]], data.frame(
    ...
    gebied = "VL",
    overlap = format(round(as.numeric(total_area / Vlaanderen_grenzen$OPPERVL),6), scientific = FALSE),
    type = "of"
  ))
  ...
  for (j in seq_along(list.dfs[1:5])) {
    ...
    selectivity <- percentage_overlap_in / (drop_units(sum(gebied$geb_area)) / sum(Vlaanderen_grenzen$OPPERVL))
    ...
    if (nrow(occ_species_intersection) != 0) {  # skip following steps if there's no overlap 
      ...
      if (j %in% c(1, 2, 3)) {
        ...
        for (row in 1:nrow(overlap_per_gebied)) {
          ...
          selectivity = percentage_overlap_in / as.numeric(overlap_per_gebied[row,]$geb_area / sum(Vlaanderen_grenzen$OPPERVL))
      ...
      if (j == 4) {
        ...
        for (row in 1: nrow(overlap_per_anb_regio)) {
          ...
          selectivity = percentage_overlap_in / as.numeric(overlap_per_anb_regio[row,]$geb_area / sum(Vlaanderen_grenzen$OPPERVL))
