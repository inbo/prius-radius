...
ps_n2khab_wgs84 <- ps_n2khab %>%
  st_transform(4326)

st_write(ps_n2khab_wgs84, "./radius/data/spatial/WGS84/ps_n2khab_wgs84.shp", append = FALSE)
```
...
for (i in unique(occ_flanders$Soort)) {
  ...
  list.results[[7]] <- rbind(list.results[[7]], data.frame(
    Soort = i, 
    ofVL = as.character(round(as.numeric(total_area / Vlaanderen_grenzen$OPPERVL),6))
  ))
  ...
  for (j in seq_along(list.dfs[1:5])) {
    ...
    if (nrow(occ_species_intersection) != 0) {  # skip following steps if there's no overlap 
      ...
      metrics_df <- data.frame(
        Soort = i,
        Gebied = paste0(c("in", "of", "sel"), names(list.dfs)[j]),
      ...
      if (j %in% c(1, 2, 3)) {
        ...
        for (row in 1:nrow(overlap_per_gebied)) {
          ...
          metrics_df <- data.frame(
            Soort = i,
            Gebied = paste0(c("in", "of", "sel"), as.character(overlap_per_gebied[row,1])),
            Overlap = format(round(c(percentage_overlap_in, percentage_overlap_of, selectivity), 6), scientific = FALSE)
          )
      ...
      if (j == 4) {
        ...
        for (row in 1: nrow(overlap_per_anb_regio)) {
          ...
          metrics_df <- data.frame(
            Soort = i,
            Gebied = paste0(c("in", "of", "sel"), as.character(overlap_per_anb_regio[row,1])),
            Overlap = format(round(c(percentage_overlap_in, percentage_overlap_of, selectivity), 6), scientific = FALSE)
          )
...
HBTRL <- list.results$HBTRL %>%
  ...
  select(Soort, contains("in"), contains("of"), contains("sel"))
...
PATDAT <- list.results$PATDAT %>%
  pivot_wider(names_from = Gebied, values_from = Overlap, values_fill = "0") %>%
  select(Soort, starts_with("n"), starts_with("in"), starts_with("of"), contains("sel"))

NBHP <- list.results$NBHP %>%
  pivot_wider(names_from = Gebied, values_from = Overlap, values_fill = "0") %>%
  select(Soort, starts_with("n"), starts_with("in"), contains("sel"))

TOP3 <- list.results$TOP3 %>%
  pivot_wider(names_from = Gebied, values_from = Overlap, values_fill = "-")

VL <- list.results$VL %>%
  mutate(ofVL = as.numeric(ofVL))
```
...
write_csv(VL, "./radius/data/output/VL.csv")
```