...
mapshot(map_1, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/punt_tot_wolk_1.png")
...
mapshot(map_2, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/punt_tot_wolk_2.png")
...
mapshot(map_3, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/punt_tot_wolk_3.png")
...
mapshot(map_4, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/punt_tot_wolk_4.png")
...
mapshot(map_5, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/punt_tot_wolk_5.png")
...
voor mate van bezetting nemen we de maximale bezettingswaarde in een habitat als weerspiegeling van de bezetting van het habitattype
```{r}
x <- N2KHAB %>%
  filter(type == "of" & gebied != "N2KHAB") %>%
  group_by(gebied) %>%
  summarise(
...
y <- y %>%
  ...
  mutate(se = ifelse(variabele == "gem_soorten", se_soorten, se_overlap),
         waarde_scaled = ifelse(variabele == "gem_overlap", waarde * schaalfactor, waarde)) 

y$groep <- factor(y$groep, levels = c("zilt en estuarium", "kustduinen", "zoetwater","heiden", "graslanden", "venen", "bossen"))

ggplot(data = y, aes(x = groep, y = waarde_scaled, fill = variabele)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.6), width = 0.5) +
  geom_errorbar(aes(ymin = waarde_scaled - se * ifelse(variabele == "gem_overlap", schaalfactor, 1), 
                    ymax = waarde_scaled + se * ifelse(variabele == "gem_overlap", schaalfactor, 1)),
                position = position_dodge(width = 0.6), width = 0.25) +
  scale_y_continuous(
    name="Aantal soorten",
    limits=c(0, 35),
    breaks=seq(0, 35, by=15),
    minor_breaks=seq(0, 35, by=5),
    expand=expansion(mult=c(0,0.01)),
    sec.axis=sec_axis(
      ~ . / scale_factor,
      name="Gemiddelde bezetting (%)",
  ...
  scale_fill_manual(values=c("gem_soorten"="#cc6999", "gem_overlap"="#e6b4ce"),
                    labels=c("gem_soorten"="Aantal soorten", 
...
# 
# 
# ggplot(y, aes(x = value, y = gebied, fill = metric)) +
#   geom_bar(stat = "identity", width = 0.7, position = position_dodge()) +
#   facet_grid(groep ~ metric, scales = "free_y", labeller = labeller(groep = facet_labels, metric = c(n_spec = "# soorten", max_overlap = "maximale bezetting (%)"))) +
#   scale_x_continuous(
#     expand = expansion(mult = c(0, 0.01)),
#     breaks = seq(0, 100, by = 10),
#     limits = c(0, 100)  
#   ) +
#   scale_fill_manual(values = c("#c04384", "#00BFC4")) +
#   custom_theme() +
#   theme(strip.placement = "outside",
#         strip.text.x.top = element_text(margin = margin(0,0,3,0), hjust = 0, vjust = 0, size = 8),
...
# ggplot(y, aes(x = value, y = gebied, fill = metric)) +
#   geom_bar(stat = "identity", width = 0.7, position = position_dodge()) +
#   facet_wrap(~ groep, scales = "free_y", ncol = 1, labeller = labeller(groep = facet_labels)) +
...
#   mutate_all(~replace(., is.na(.), 0))
# 
# scale_factor <- max(y$n_spec) / max(y$max_overlap)
# 
# ratio <- max(y$n_spec) / max(y$max_overlap)
# 
# ggplot(data = y, aes(x = reorder(gebied, n_spec))) +
#   geom_bar(aes(y = n_spec), stat = "identity", fill = "skyblue") +
#   geom_line(aes(y = max_overlap * ratio), color = "red", size = 1, group = 1) +
#   geom_point(aes(y = max_overlap * ratio), color = "red", size = 1) +
#   scale_y_continuous(
#     name = "Number of Species",
#     sec.axis = sec_axis(~./ratio, name = "Maximum Overlap")
#   ) +
#   theme_minimal() +
#   theme(
...
#ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/NBHP/nbhp_bezetting_top5.png", width = 23, height = 10, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/NBHP/nbhp_top10_voorkomen.png", 
       width = 23, height = 15, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/PATDAT/patdat_nspec_per_br.png", width = 23, height = 10, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/PATDAT/patdat_bezetting_top10.png",
       width = 23, height = 10, units = "cm", dpi=300)
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/PATDAT/patdat_bezetting_top5.png", 
#        width = 23, height = 10, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/PATDAT/patdat_spec_top5.png",
       width = 23, height = 10, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/PATDAT/patdat_ndom_per_recht.png", 
       width = 23, height = 10, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren/Resultaten/PATDAT/patdat_perc_per_br.png",
...
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = clusters_sbp, aes(fill = factor(CLUSTERNR)), color = "black",linewidth = 0.1,  alpha = 0.9) +
...
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = lu_sbp_pls %>% filter(soort == "Kleine Modderkruiper"), 
  ...
  #scale_fill_manual(values = c("Kleine Modderkruiper" = "#66C2A5")) +
  theme_map() +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
...
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", color = "black", linewidth = 0.2) +
  geom_sf(data = data$geom, aes(fill = data$n_spec), color = "grey20", linewidth = 0.05) +
