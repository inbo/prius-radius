...
custom_theme <- function() {
  theme_few() +
    theme(
      ...
      strip.text = element_text(size = 10, face = "bold", hjust = -0.01, family = "Arial"),
...
sbp_pls_intersects <- st_read("./radius/data/spatial/sbp_pls_intersects.shp")

## metrics
SBP_pgs <- read_csv("./radius/data/output/SBP_pgs_long.csv")
SBP_pls <- read_csv("./radius/data/output/SBP_pls_long.csv")
SBP_pls_extra <- read_csv("./radius/data/output/SBP_pls_extra.csv")

SBP_pgs_wide <- read_csv("./radius/data/output/SBP_pgs_wide.csv")
SBP_pls_wide <- read_csv("./radius/data/output/SBP_pls_wide.csv")
```

# FIGUUR PUNT TOT WOLK
```{r figuur-punt-tot-wolk, echo=FALSE}
bbox <- st_bbox(c(xmin = 4.557773, ymin = 50.987674, 
                  xmax = 4.577814, ymax = 50.999349), 
                crs = st_crs(4326))

bbox <- st_as_sfc(bbox)

occ_data <- occ_flanders %>%
  filter(Soort == "Canadese guldenroede") %>%
  st_intersection(bbox) %>%
  mutate(id = row_number())


occ_data_buf_40 <- occ_data %>%
  st_transform(31370) %>%
  st_buffer(dist = 40) 

occ_data_buf_80 <- occ_data %>%
  st_transform(31370) %>%
  st_buffer(dist = 80) 

occ_data_buf_100 <- occ_data %>%
  st_transform(31370) %>%
  st_buffer(dist = 100) 

occ_data_union <- occ_data_buf_100 %>%
  st_union() %>%
  st_cast('POLYGON') %>% 
  st_sf()
  
occ_data_intersect <- occ_data_union %>%
  st_intersection(ps_hbtrl %>% st_transform(31370))

leaflet() %>%
  addTiles(data = "OSM") %>%
  addCircleMarkers(data = occ_data %>% st_transform(4326), radius = 2, opacity = 1, fillOpacity = 1, label = ~id) %>%
  fitBounds(
    lng1 = 4.566793, lat1 = 50.988674, 
    lng2 = 4.568793, lat2 = 50.996349  
  )  

map_1 <- leaflet() %>%
  addTiles(data = "OSM") %>%
  addCircleMarkers(data = occ_data %>% st_transform(4326), radius = 2, opacity = 1, fillOpacity = 1, label = ~id) %>%
  addPolygons(data = occ_data_buf_40 %>% st_transform(4326) %>% filter(id %in% c(2,7,8,11)), fillOpacity = 0.3, weight = 1.5) %>%
  addPolygons(data = occ_data_buf_80 %>% st_transform(4326) %>% filter(id %in% c(1,9,10,12,13)), fillOpacity = 0.3, weight = 1.5) %>%
  addPolygons(data = occ_data_buf_100 %>% st_transform(4326) %>% filter(id %in% c(3,4,5,6,14)), fillOpacity = 0.3, weight = 1.5) %>%
  fitBounds(
    lng1 = 4.566793, lat1 = 50.988674, 
    lng2 = 4.568793, lat2 = 50.996349  
  )
...
#mapshot(map_1, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/punt_tot_wolk_1.png")

map_2 <- leaflet() %>%
  addTiles(data = "OSM") %>%
  addCircleMarkers(data = occ_data %>% st_transform(4326), radius = 2, opacity = 1, fillOpacity = 1, label = ~id) %>%
  addPolygons(data = occ_data_buf_100 %>% st_transform(4326), fillOpacity = 0.3, weight = 1.5) %>%
  fitBounds(
    lng1 = 4.566793, lat1 = 50.988674, 
    lng2 = 4.568793, lat2 = 50.996349  
  ) 

#mapshot(map_2, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/punt_tot_wolk_2.png")

map_3 <- leaflet() %>%
  addTiles(data = "OSM") %>%
  addPolygons(data = occ_data_union %>% st_transform(4326), weight = 1.5, fillOpacity = 0.3) %>%
  fitBounds(
    lng1 = 4.566793, lat1 = 50.988674, 
    lng2 = 4.568793, lat2 = 50.996349  
  ) 

#mapshot(map_3, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/punt_tot_wolk_3.png")

map_4 <- leaflet() %>%
  addTiles(data = "OSM") %>%
  addPolygons(data = ps_hbtrl %>% st_transform(4326), color = "#a4e98f", fill = "#a4e98f", fillOpacity = 0.8, weight = 1) %>%
  addPolygons(data = occ_data_union %>% st_transform(4326), weight = 1.5, fillOpacity = 0.3) %>%
  fitBounds(
    lng1 = 4.566793, lat1 = 50.988674, 
    lng2 = 4.568793, lat2 = 50.996349  
...
#mapshot(map_4, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/punt_tot_wolk_4.png")

map_5 <- leaflet() %>%
  addTiles(data = "OSM") %>%
  addPolygons(data = ps_hbtrl %>% st_transform(4326), color = "#a4e98f", fill = "#a4e98f", fillOpacity = 0.8, weight = 1) %>%
  addPolygons(data = occ_data_union %>% st_transform(4326), weight = 1.5, fillOpacity = 0.4) %>%
  addPolygons(data = occ_data_intersect %>% st_transform(4326), weight = 1.5, color = "red", fillOpacity = 0.3) %>%
  fitBounds(
    lng1 = 4.566793, lat1 = 50.988674, 
    lng2 = 4.568793, lat2 = 50.996349  
  )

#mapshot(map_5, file = "G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/punt_tot_wolk_5.png")
```
...
VL <- VL %>%
  ...
  select(soort, species, Groep, overlap) %>%
  distinct() %>%
  mutate(Groep = recode(Groep, "plant" = "planten", "dier" = "dieren"))


ggplot(VL, aes(x = (overlap * 100) + 0.04, y = reorder(soort, overlap))) +
  geom_bar(stat = "identity", show.legend = FALSE, width = 0.7, fill = "#c04384") +  
  facet_wrap( ~ Groep, scales = "free_y", ncol = 1, strip.position = "right") +  
  scale_x_continuous(
    expand = expansion(mult = c(0, 0.05)),
    labels = scales::percent_format(scale = 1)) +
  labs(x = "Bezetting van Vlaanderen", y = NULL) +
  custom_theme() + 
  theme(
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VL/vl_bezetting_update.png", width = 23, height = 20, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/vl_bezetting_update.png", width = 23, height = 20, units = "cm", dpi=300)


ggplot(VL, aes(x = reorder(soort, overlap), y = (overlap * 100) + 0.04)) +
  geom_bar(stat = "identity", show.legend = FALSE, width = 0.7, fill = "#c04384") +  
  scale_y_continuous(
    expand = expansion(mult = c(0, 0.05)),
    labels = scales::percent_format(scale = 1)) +
  labs(y = "Bezetting van Vlaanderen", x = NULL) +
  custom_theme() + 
  theme(strip.placement = "outside",
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0),
        )

# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VL/vl_bezetting2.png", 
#        width = 23, height = 13, units = "cm", dpi=300)
# ggsave("./radius/data/output/figuren/vl_bezetting2.png", 
#        width = 23, height = 13, units = "cm", dpi=300)
```
...
## verspreiding van soorten in Vlaanderen
```{r}
for (species in c("beverrat", "Noord-Aziatische modderkruiper")) {
  print(leaflet() %>%
    addTiles(data = "OSM") %>%
    addPolygons(data = ps_nbhp %>% st_transform(4326), color = "green", fill = "green") %>%
    addCircleMarkers(data = occ_flanders %>% filter(Soort == species) %>% st_transform(4326)))
}
```
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/HBTRL/hbtrl_nspec_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/hbtrl_nspec_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
```
...
x <- HBTRL %>%
  ...
  mutate(gebied_label = paste0(naam, "\n (", gebied, ")"))
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/HBTRL/hbtrl_spec_per_gebied_update.png", 
       width = 23, height = 17, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/hbtrl_spec_per_gebied_update.png", 
       width = 23, height = 17, units = "cm", dpi=300)
```
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/HBTRL/hbtrl_bezetting_update.png", 
       width = 23, height = 27, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/hbtrl_bezetting_update.png", 
       width = 23, height = 27, units = "cm", dpi=300)
```
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/HBTRL/hbtrl_in_vs_of_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/hbtrl_in_vs_of_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/HBTRL/hbtrl_Up_Cn_Tsi_update.png", 
       width = 23, height = 20, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/hbtrl_Up_Cn_Tsi_update.png", 
       width = 23, height = 20, units = "cm", dpi=300)
```
...
species_vglrl <- c("treurmaina","nijlgans","beverrat","wasbeerhond","muskusrat","rosse stekelstaart","wasbeer","heilige ibis","Canadese gans","Amerikaanse nerts", "watercrassula", "kleine waterteunisbloem", "grote waterteunisbloem")
...
VGLRL_subset_nspec <- VGLRL_subset_nspec %>%
  mutate(number_bin_subset = cut(total_species_subset, 
...
colors <- c("1-2" = "#f2d9e6", "3-4" = "#e6b4ce", "5-6" = "#d98eb5", "7-8" = "#cc6699", "9-10" = "#c04384")
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VGLRL/vglrl_nspec_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/vglrl_nspec_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VGLRL/vglrl_spec_per_gebied_update.png", 
       width = 23, height = 17, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/vglrl_spec_per_gebied_update.png", 
       width = 23, height = 17, units = "cm", dpi=300)
```
...
ggplot(x, aes(x = ifelse(overlap < 1, overlap * 100, overlap), y = soort, fill = overlap)) +
  ...
  facet_grid(groep ~ type, scales = "free", space = "free_y") +  
  scale_x_continuous(
    expand = expansion(mult = c(0, 0.05))) +  
  scale_y_discrete(labels = function(soort) {
    sapply(soort, function(s) {
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VGLRL/vglrl_bezetting_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/vglrl_bezetting_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
```
...
ggplot() +
  geom_text(data = in_vs_of_VGLRL_subset, aes(x = overlap.in*100, y = overlap.of*100, label = abbr), size = 4, family = "Arial") +
  coord_trans(y = "log2") +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
  scale_x_continuous(labels = scales::percent_format(scale = 1), limits = c(0,100)) + 
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VGLRL/vglrl_in_vs_of_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/vglrl_in_vs_of_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
df_Lp <- VGLRL_spatial %>%
  dplyr::filter(soort == "kleine waterteunisbloem" & type == "in" & gebied != "VGLRL" & overlap != 0) %>%
  st_cast("MULTIPOLYGON")

occs_Lp <- occ_flanders %>%
  dplyr::filter(Soort == "kleine waterteunisbloem")

map_Lp <- ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = ps_vglrl, fill = "grey80", color = "grey80", size = 0.5) +
  geom_sf(data = df_Lp, aes(text = paste0(gebnaam, " (", gebied, "): ", round(overlap * 100, 1), "%")), fill = "#c04384", color = "#c04384", size = 0.5) +
  geom_sf(data = occs_Lp, color = "blue", size = 1) +
  theme_map()

map_Lp

barplot_Lp <- ggplot() +
  geom_bar(data = df_Lp, aes(x = overlap, y = reorder(gebied, overlap)),
           stat = "identity", colour = "#c04384", fill = "#c04384", width = 0.7) +
  custom_theme() +
  theme(axis.text.x = element_text(angle = 90),
        axis.title.y = element_blank(),
...
left_column <- (barplot_Lp + labs(tag = "a.")) /
  (barplot_Oj + labs(tag = "c.")) / 
  (barplot_Ta + labs(tag = "e.")) +
  plot_layout(heights = c(1, 1, 1))

right_column <- (map_Lp + labs(tag = "b.")) / 
  (map_Oj + labs(tag = "d.")) / 
  (map_Ta + labs(tag = "f.")) +
  plot_layout(heights = c(1, 1, 1))
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/VGLRL/vglrl_Oj_Ta_update.png", 
       width = 23, height = 17, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/vglrl_Oj_Ta_update.png", 
       width = 23, height = 17, units = "cm", dpi=300)
```
...
ggplot(N2KHAB_nspec_pertype, aes(x = reorder(gebied, n_spec), y = n_spec)) +
  ...
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5),
        ...
        strip.text.x.top = element_text(margin = margin(0,0,3,0), hjust = 0, vjust = 0),
...
ggplot(N2KHAB_nspec_pertype, aes(x = reorder(gebied, n_spec), y = n_spec, fill = EU_lijst)) +
  ...
  facet_grid(~ groep, scales = "free_x", space = "free_x", labeller = labeller(groep = facet_labels)) +
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_nspec_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_nspec_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
##### Mate van bezetting vs aantal soorten

voor mate van bezetting nemen we de gemiddelde bezettingswaarde in een habitat als weerspiegeling van de bezetting van het habitattype
```{r}
x <- N2KHAB %>%
  filter(type == "of" & gebied != "N2KHAB" & overlap != 0) %>%
  group_by(gebied) %>%
  summarise(
    max_overlap = max(overlap) * 100, 
    mean_overlap = mean(overlap) * 100
  )
...
y <- N2KHAB %>%
  ...
  summarise(n_spec = n()) %>%
  ungroup() %>%
  # add_row(groep = "kustduinen", gebied = "2150", n_spec = 0) %>%
  # add_row(groep = "bossen", gebied = "91F0", n_spec = 0) %>%
  left_join(x, by = "gebied") %>%
  mutate_all(~replace(., is.na(.), 0)) %>%
  pivot_longer(cols = c(n_spec, mean_overlap), names_to = "metric") 

y_order <- y %>%
  dplyr::filter(metric == "n_spec") %>%
  arrange(value) %>%
  pull(gebied)

y$gebied <- factor(y$gebied, levels = y_order)
y$metric <- factor(y$metric, levels = c("n_spec", "mean_overlap"))
y$groep <- factor(y$groep, levels = c("zilt en estuarium", "kustduinen", "zoetwater","heiden", "graslanden", "venen", "bossen"))
...
y <- y %>%
  mutate(value = ifelse(metric == "mean_overlap", value * scale_factor, value))

ggplot(data = y, aes(x = gebied, y = value, fill = metric)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9), width = 0.8) +
  facet_grid(~groep, scales="free_x", space="free_x", labeller=labeller(groep=facet_labels)) +
  scale_y_continuous(
    name="Aantal soorten",
    limits=c(0, 65),  
    breaks=seq(0, 65, by=15),  
    minor_breaks=seq(0, 65, by=5), 
    expand=expansion(mult=c(0,0.01)),
    sec.axis=sec_axis(
      ~ . / scale_factor,
  ...
  scale_fill_manual(values=c("n_spec"="#cc6999", "mean_overlap"="#e6b4ce"),
                    labels=c("n_spec"="Aantal soorten", 
                             "mean_overlap"="Gemiddelde bezetting (%)")) +
  labs(x="Habitat") +
  custom_theme() +
  theme(
    ...
    legend.position="bottom",
    legend.title=element_blank(), 
    panel.grid.major.y=element_line(color="grey75", linewidth=0.5), 
    panel.grid.minor.y=element_line(color="grey90", linewidth=0.25)
)

ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_bezetting_per_type_update.png", 
       width = 23, height = 13, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/2khab_bezetting_per_type_update.png", 
       width = 23, height = 13, units = "cm", dpi=300)

### Bezetting per habitatgroep
y <- N2KHAB %>%
  dplyr::filter(gebied != "N2KHAB" & type == "of" & overlap != 0) %>%
  left_join(species_list, by = c("soort" = "Soort"), relationship = "many-to-many") %>%
  left_join(habitats, by = c("gebied" = "code")) %>%
  ...
  group_by(groep) %>%
  summarise(
    gem_soorten = mean(n_spec),
    se_soorten = sd(n_spec) / sqrt(n()),
    gem_overlap = mean(mean_overlap),
    se_overlap = sd(mean_overlap) / sqrt(n())
  ) 

scale_factor <- max(y$gem_soorten, na.rm = TRUE) / max(y$gem_overlap, na.rm = TRUE)

y <- y %>%
  pivot_longer(cols = c(gem_soorten, gem_overlap),
               names_to = "variabele",
               values_to = "waarde") %>%
  mutate(se = ifelse(variabele == "gem_soorten", se_soorten, se_overlap),
         waarde_scaled = ifelse(variabele == "gem_overlap", waarde * scale_factor, waarde)) 

y$groep <- factor(y$groep, levels = c("zilt en estuarium", "kustduinen", "zoetwater","heiden", "graslanden", "venen", "bossen"))

ggplot(data = y, aes(x = groep, y = waarde_scaled, fill = variabele)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.6), width = 0.5) +
  geom_errorbar(aes(ymin = waarde_scaled - se * ifelse(variabele == "gem_overlap", scale_factor, 1), 
                    ymax = waarde_scaled + se * ifelse(variabele == "gem_overlap", scale_factor, 1)),
                position = position_dodge(width = 0.6), width = 0.15, linewidth = 0.3, color = "black") +
  scale_y_continuous(
    name="Gemiddeld aantal soorten",
    limits=c(0, 40),
    breaks=seq(0, 40, by=15),
  ...
  scale_fill_manual(values=c("gem_soorten"="#cc6999", "gem_overlap"="#e6b4ce"),
                    labels=c("gem_soorten"="Gemiddeld aantal soorten", 
                             "gem_overlap"="Gemiddelde bezetting (%)")) +
  labs(x="Habitat") +
  custom_theme() +
  theme(
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_bezetting_per_groep_update.png", 
       width = 23, height = 13, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/2khab_bezetting_per_groep_update.png", 
       width = 23, height = 13, units = "cm", dpi=300)
```
...
ggplot() +
  geom_sf(data = Provincies_grenzen, fill= "#f0f0f0", color = "black", linewidth = 0.2) +
  geom_sf(data = N2KHAB_nspec, aes(color = number_bin, fill = number_bin)) +
  labs(fill = "Aantal IUS", color = "Aantal IUS") +
  scale_fill_manual(values = colors) +
  scale_color_manual(values = colors) +
  theme_map() +
...
x <- N2KHAB %>%
  dplyr::filter(gebied %in% c("91E0", "4010", "7140", "7110") & type == "of" & overlap != 0) %>%
  left_join(habitats, by = c("gebied" = "code")) %>%
  left_join(species_list, by = c("soort" = "Soort")) %>%
  select(soort, Groep, gebied, habitat, groep, overlap) %>%
  distinct() %>%
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_bezetting_update.png", 
       width = 23, height = 27, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_bezetting_update.png", 
       width = 23, height = 27, units = "cm", dpi=300)
```
...
in_N2KHAB <- N2KHAB %>%
  dplyr::filter(gebied == "N2KHAB" & type == "in" & overlap != 0)  
...
ggplot() +
  geom_text(data = in_vs_of_N2KHAB, aes(x = overlap.in*100, y = overlap.of*100, label = abbr), size = 4, family = "Arial") +
  coord_trans(y = "log2") +
  scale_y_continuous(labels = scales::percent_format(scale = 1)) + 
  scale_x_continuous(labels = scales::percent_format(scale = 1), limits = c(0,100)) + 
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_in_vs_of_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_in_vs_of_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
ggplot(data = x, aes(x = reorder_within(gebied, overlap, groep), y = overlap * 100)) +
  geom_bar(stat = "identity", width = 0.7, fill = "#cc6999") +
  facet_grid(soort ~ groep, scales = "free", space = "free_x", labeller = labeller(groep = facet_labels)) +
  labs(x="Habitat", y = "% van totale verspreidingsoppervlak") +
  scale_x_reordered() +
  ...
  theme(
    ...
    legend.position="bottom",
    legend.title=element_blank(), 
    strip.text.x = element_text(hjust = 0, vjust = 0, color = "black"),
    strip.placement = "outside",
    #strip.background.x = element_rect(fill = "grey98", color = NA),
    panel.spacing.x = unit(0.5, "lines"),
    plot.margin = margin(r = 5, unit = "mm")
  )

# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_gen_vs_spec.png", 
#        width = 23, height = 17, units = "cm", dpi=300)
# ggsave("./radius/data/output/figuren/n2khab_gen_vs_spec.png", 
#        width = 23, height = 17, units = "cm", dpi=300)
```

```{r n2khab-vglk-spec-ZT, echo = FALSE, warning=FALSE}
x <- N2KHAB %>%
  dplyr::filter(gebied != "N2KHAB" & type == "in" & soort %in% c("Leidse plant", "moerasaronskelk", "watercrassula")) %>%
  left_join(habitats, by = c("gebied" = "code")) 
...
ggplot(data = x, aes(x = reorder_within(gebied, overlap, groep), y = ifelse(overlap != 0 & overlap < 0.05, overlap * 100 + 0.1, overlap * 100))) +
  geom_bar(stat = "identity", width = 0.7, fill = "#cc6999") +
  facet_grid(soort ~ groep, scales = "free_x", space = "free_x", labeller = labeller(groep = facet_labels)) +
  labs(x="Habitat", y = "% van totale verspreidingsoppervlak") +
  scale_x_reordered() +
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)), labels = scales::percent_format(scale = 1)) +
  custom_theme() +
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_gen_vs_spec.png", 
#        width = 23, height = 17, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_vgl_ZT_pl.png",
       width = 23, height = 17, units = "cm", dpi=300)
```

```{r n2khab-vglk-spec-zoogdieren, echo = FALSE, warning=FALSE}
x <- N2KHAB %>%
  dplyr::filter(gebied != "N2KHAB" & type == "in" & soort %in% c("wasbeer","Chinese muntjak", "sikahert", "Siberische grondeekhoorn")) %>%
  left_join(habitats, by = c("gebied" = "code")) 
...
ggplot(data = x, aes(x = reorder_within(gebied, overlap, groep), y = ifelse(overlap != 0 & overlap < 0.05, overlap * 100 + 1, overlap * 100))) +
  geom_bar(stat = "identity", width = 0.7, fill = "#cc6999") +
  facet_grid(soort ~ groep, scales = "free_x", space = "free_x", labeller = labeller(groep = facet_labels)) +
  labs(x="Habitat", y = "% van totale verspreidingsoppervlak") +
  scale_x_reordered() +
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_gen_vs_spec.png", 
#        width = 23, height = 17, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_vgl_zoogd.png",
       width = 23, height = 17, units = "cm", dpi=300)
```

```{r n2khab-vglk-spec-terr-planten, echo = FALSE, warning=FALSE}
x <- N2KHAB %>%
  dplyr::filter(gebied != "N2KHAB" & type == "in" & soort %in% c("reuzenberenklauw", "reuzenbalsemien","rimpelroos","late guldenroede")) %>%
  left_join(habitats, by = c("gebied" = "code")) 
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_gen_vs_spec.png", 
#        width = 23, height = 17, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_vgl_terr_pl.png",
       width = 23, height = 17, units = "cm", dpi=300)
```

```{r n2khab-vglk-spec-terr-planten, echo = FALSE, warning=FALSE}
x <- N2KHAB %>%
  dplyr::filter(gebied != "N2KHAB" & type == "in" & soort %in% c("moerasaronskelk", "watercrassula", "rimpelroos", "reuzenbalsemien", "reuzenberenklauw")) %>%
  left_join(habitats, by = c("gebied" = "code"))

x$groep <- factor(x$groep, levels = c("zilt en estuarium", "kustduinen", "zoetwater", "heiden", "graslanden", "venen", "bossen"))
x$soort <- factor(x$soort, levels = c("moerasaronskelk", "watercrassula", "rimpelroos", "reuzenbalsemien", "reuzenberenklauw"))
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/N2KHAB/n2khab_vgl_species_update.png",
       width = 23, height = 20, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/n2khab_vgl_species_update.png",
       width = 23, height = 20, units = "cm", dpi=300)
```
...
ggplot(x, aes(x = reorder(gebied, overlap), y = overlap * 100)) +
  geom_bar(stat = "identity", fill = "#c04384") +
  facet_grid(soort ~ groep, scales = "free_x", switch = "y", labeller = labeller(groep = facet_labels)) +
...
(sum(ps_nbhp$opp_ha) / (Vlaanderen_grenzen$OPPERVL / 10000)) * 100
```
...
Er zijn enorm veel natuurbeheerplannen in Vlaanderen (in totaal 2125). Deze nemen 0.09% van het Vlaamse landoppervlak in.
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/NBHP/NBHP_nspec_per_type_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/nbhp_nspec_per_type_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
x <- NBHP %>%
  dplyr::filter(type == "n_per_type") %>%
  group_by(gebied) %>%
  slice_max(n = 20, order_by = overlap) %>%
...
x <- NBHP %>%
  ...
  select(soort, Groep, overlap, n_geb, label_color) %>%
  group_by(Groep) %>%
  slice_max(order_by = overlap, n = 5) %>%
  pivot_longer(cols = c("overlap", "n_geb"), names_to = "type", values_to = "overlap") %>% 
  mutate(type = recode(type, "overlap" = "% bezetting", "n_geb" = "# natuurbeheerplannen")) %>% 
  replace_na(list(overlap = 0)) 
...
x$soort <- factor(x$soort, levels = x_order)
x$type <- factor(x$type, levels = c("% bezetting", "# natuurbeheerplannen"))
x$Groep <- factor(x$Groep, levels = c("dieren", "planten"))
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/NBHP/nbhp_bezetting_top10_update.png", width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/nbhp_bezetting_top10_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
```
...
n_nbhp <- NBHP %>%
  dplyr::filter(gebied == "NBHP" & type == "n_nbhp") %>%
  rename("n_nbhp" = "overlap") %>%
  select(soort, species, n_nbhp)
...
ggplot(n_nbhp , aes(x = reorder(soort, overlap), y = overlap, fill = )) +
  geom_bar(stat = "identity", fill = "#c04384", aes(text = paste0(soort, " (", overlap, " natuurbeheerplannen)"))) +
  scale_y_continuous(expand = expansion(mult = c(0,0.01))) +
  custom_theme() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1),
...
in_NBHP <- NBHP %>%
  ...
  left_join(n_nbhp, by = c("soort", "species")) %>%
  pivot_longer(cols = c("overlap", "n_nbhp"), names_to = "type", values_to = "overlap") %>% 
  mutate(type = recode(type, "overlap" = "% in natuurbeheerplannen", "n_nbhp" = "# natuurbeheerplannen")) %>% 
  replace_na(list(overlap = 0)) 

in_NBHP_order <- in_NBHP %>%
  filter(type == "% in natuurbeheerplannen") %>%
  arrange(overlap) %>%
  pull(soort)

in_NBHP$soort <- factor(in_NBHP$soort, levels = in_NBHP_order)
in_NBHP$type <- factor(in_NBHP$type, levels = c("% in natuurbeheerplannen", "# natuurbeheerplannen"))
in_NBHP$Groep <- factor(in_NBHP$Groep, levels = c("dieren", "planten"))

#label_colors <- setNames(in_NBHP$label_color, in_NBHP$soort)

ggplot(in_NBHP, aes(x = ifelse(overlap < 1, overlap * 100, overlap), y = soort, fill = overlap)) +
  geom_bar(stat = "identity", show.legend = FALSE, width = 0.7, fill = "#c04384") +  
  geom_vline(data = . %>% filter(type == "% in natuurbeheerplannen"), 
             aes(xintercept = 50), 
  ...
  scale_x_continuous(
    expand = expansion(mult = c(0, 0.05)),
    limits = function(x) {
      if(all(is.na(x))) return(c(0, 100))
      if(max(x, na.rm = TRUE) <= 100) c(0, 100) else range(x, na.rm = TRUE)
    },
    labels = function(x) {
      if(all(is.na(x))) return(x)
      if(max(x, na.rm = TRUE) <= 100) paste0(x, "%") else x
    }
  ) +  
  labs(x = "Voorkomen in Natuurbeheerplannen", y = NULL) +
  custom_theme() + 
  theme(strip.placement = "outside",
        axis.text.y = element_markdown(size = 10),
        strip.text.x.top = element_text(margin = margin(0,0,3,0), hjust = 0, vjust = 0, size = 10))

# ggplot(in_NBHP %>% slice_max(order_by = overlap, n = 10),aes(x = overlap * 100, y = reorder(soort, overlap))) +
#   geom_bar(stat = "identity", colour = "white", linewidth = 0.01, fill = "#c04384", width = 0.7) +
#   facet_grid(~ type) +
#   scale_x_continuous(labels = scales::percent_format(scale = 1), limits = c(0,100), expand = expansion(mult = c(0,0))) + 
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/NBHP/nbhp_top10_voorkomen_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/nbhp_top10_voorkomen_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
in_vs_of_NBHP <- in_NBHP %>%
  select(!type) %>%
  left_join(of_NBHP %>% select(!type), by = c("soort", "species"), suffix = c(".in", ".of")) %>%
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/NBHP/nbhp_in_vs_of_update.png", 
#        width = 23, height = 15, units = "cm", dpi=300)
# ggsave("./radius/data/output/figuren/nbhp_in_vs_of_update.png", 
#        width = 23, height = 15, units = "cm", dpi=300)
```
...
soorten <- c("Siberische grondeekhoorn", "kleine waterteunisbloem", "Chinese moerasslak", "sikahert", "rosse stekelstaart", "beverrat", "marmerkreeft", "Noord-Aziatische modderkruiper", "waterhyacint", "watercrassula")
...
data_filtered <- data %>%
  ...
  bind_rows(
    data %>%
      ...
      mutate(gebied = "Buiten natuurbeheerplannen", overlap = buiten_nbhp) %>%
      select(-buiten_nbhp))

group_order <- c("Buiten natuurbeheerplannen", 
...
color_mapping <- c(
  "Buiten natuurbeheerplannen" = "grey60",  
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/NBHP/nbhp_resp_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/nbhp_resp_update.png", 
       width = 23, height = 15, units = "cm", dpi=300)
```
...
volgorde_br <- c(
  ...
  "Brabantse Wouden",
  "Taxandria",
  "Demerland & Zuiderkempen",
  "Lage tot aan Hoge Kempen",
  "Hoge Kempen tot Voeren"
)
```
...
#### Aandeel van Vlaanderen dat wordt ingenomen door ANB patrimonium?
```{r overview-perc-nbhp-flanders, echo=FALSE}
am_patdat <- am_patdat %>%
  mutate(opp = st_area(.))

(sum(am_patdat$opp) / (Vlaanderen_grenzen$OPPERVL)) * 100
```

#### Verdeling van rechten anb binnen deze domeinen?
```{r overview-patdat-type-beheerder, echo=FALSE}
patdat_summary <- am_patdat %>%
  mutate(opp = st_area(geometry)) %>%
  st_drop_geometry() %>%
  group_by(rchtnnb) %>%
...
plot_ly(patdat_summary, labels = ~rchtnnb, values = ~area, type = 'pie',
        textinfo = 'label+percent',
        insidetextorientation = 'radial',
        marker = list(colors = custom_colors)) %>%
  layout(showlegend = FALSE)
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/patdat_nspec_per_br_update.png", width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/patdat_nspec_per_br_update.png",
       width = 23, height = 10, units = "cm", dpi=300)
...
#### Welke soorten komen in welke mate voor in ANB domeinen? {.tabset}

##### Inclusief openbare bossen
```{r spec-of-patdat-incl-ob, echo=FALSE}
PATDAT_ngeb <- PATDAT %>%
...
x <- PATDAT %>%
  ...
  mutate(type = recode(type, "overlap" = "% bezetting", "n_domeinen" = "# domeinen")) %>%
  replace_na(list(overlap = 0))
...
x$soort <- factor(x$soort, levels = x_order)
x$type <- factor(x$type, levels = c("% bezetting", "# domeinen"))
x$Groep <- factor(x$Groep, levels = c("dieren", "planten"))
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/patdat_bezetting_top10_update.png",
#        width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/patdat_bezetting_top10_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
```

##### Exclusief openbare bossen 
```{r spec-of-patdat-excl-ob, echo=FALSE}
PATDAT_ngeb <- PATDAT_excl %>%
  dplyr::filter(gebied == "PATDAT_excl_ob" & type == "n_domeinen") %>%
  transmute(soort, "n_domeinen" = overlap )

x <- PATDAT_excl %>%
  dplyr::filter(gebied == "PATDAT_excl_ob" & type == "of")  %>%
  mutate(label_color = ifelse(overlap == 0, "grey75", "black")) %>%
  left_join(species_list, by = c("soort" = "Soort")) %>%
  mutate(EU_color = ifelse(EU_lijst != 0, "red", "black")) %>%
  select(soort, species, Groep, overlap, EU_color, label_color) %>%
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/patdat_bezetting_top5.png", 
#        width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/patdat_bezetting_top5.png", 
       width = 23, height = 10, units = "cm", dpi=300)
```
...
label_colors <- setNames(of_PATDAT$label_color, of_PATDAT$soort)

# Plot met aangepaste kleuren voor x-as labels
ggplotly(
  ggplot(of_PATDAT , aes(x = reorder(soort, overlap), y = overlap)) +
...
ggplotly(
  ggplot(in_PATDAT,aes(x = reorder(soort, overlap), y = overlap * 100)) +
    geom_bar(stat = "identity", colour = "white", linewidth = 0.01, fill = "#c04384", width = 0.7) +
    scale_y_continuous(labels = scales::percent_format(scale = 1), expand = c(0, 0.1)) + 
    labs(y = "Bezetting van ANB patrimonium (%)") +
    custom_theme() +
    theme(
      axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1),
      axis.title.x = element_blank()
...
ggplot(in_PATDAT %>% slice_max(order_by = overlap, n = 10),aes(x = overlap * 100, y = reorder(soort, overlap))) +
  geom_bar(stat = "identity", colour = "white", linewidth = 0.01, fill = "#c04384", width = 0.7) +
  scale_x_continuous(labels = scales::percent_format(scale = 1), limits = c(0,100), expand = expansion(mult = c(0,0))) + 
  labs(x = "Percentage van totale verspreiding") +
  custom_theme() +
...
PATDAT_ngeb_per_recht <- PATDAT %>%
  dplyr::filter(type == "anb_rechten") 

ggplot(data = PATDAT_ngeb_per_recht %>% filter(soort %in% c("nijlgans", "Canadese gans", "Amerikaanse vogelkers", "Aziatische hoornaar (soort)", "muskusrat", "lettersierschildpad", "reuzenbalsemien", "invasieve duizendknopen", "late guldenroede", "watercrassula")), aes(x= overlap, y = soort, fill = gebied)) +
         geom_col()


in_PATDAT <- PATDAT %>%
  dplyr::filter(gebied == "PATDAT_incl_ob" & type == "in" & overlap != 0) %>%
  left_join(species_list, by = c("soort" = "Soort")) %>%
  select(soort, species, Groep, overlap) %>%
  distinct() %>%
  mutate(Groep = recode(Groep, "plant" = "planten", "dier" = "dieren")) %>%
  ...
  left_join(PATDAT_ngeb, by = "soort") %>%
  pivot_longer(cols = c("overlap", "n_domeinen"), names_to = "type", values_to = "overlap") %>% 
  mutate(type = recode(type, "overlap" = "% in ANB-patrimonium", "n_domeinen" = "# ANB-domeinen")) %>% 
  replace_na(list(overlap = 0)) 

in_PATDAT_order <- in_PATDAT %>%
  filter(type == "% in ANB-patrimonium") %>%
  arrange(overlap) %>%
  pull(soort)

in_PATDAT$soort <- factor(in_PATDAT$soort, levels = in_PATDAT_order)
in_PATDAT$type <- factor(in_PATDAT$type, levels = c("% in ANB-patrimonium", "# ANB-domeinen"))
in_PATDAT$Groep <- factor(in_PATDAT$Groep, levels = c("dieren", "planten"))

#label_colors <- setNames(in_NBHP$label_color, in_NBHP$soort)

ggplot(in_PATDAT, aes(x = ifelse(overlap < 1, overlap * 100, overlap), y = soort, fill = overlap)) +
  geom_bar(stat = "identity", show.legend = FALSE, width = 0.7, fill = "#c04384") +  
  geom_vline(data = . %>% filter(type == "% in ANB-patrimonium"),
             aes(xintercept = 50),
             linetype = "dashed", color = "black", size = 0.5) +
  facet_grid(~ type, scales = "free_x") +  
  scale_x_continuous(
  ...
  labs(x = "Voorkomen in ANB-patrimonium", y = NULL) +
  custom_theme() + 
  theme(strip.placement = "outside",
        axis.text.y = element_markdown(size = 9),
        strip.text.x.top = element_text(margin = margin(0,0,3,0), hjust = 0, vjust = 0, size = 10))

# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/patdat_spec_top5_update.png",
#        width = 23, height = 10, units = "cm", dpi=300)
# ggsave("./radius/data/output/figuren/patdat_spec_top5_update.png", 
#        width = 23, height = 10, units = "cm", dpi=300)

```

```{r}
PATDAT_spatial <- am_patdat %>%
  right_join(VGLRL, by = c("na2000code" = "gebied"), keep = TRUE)
...
beheerregios <- st_read("G:/Mijn Drive/Fleur/RadIUS/ANB Terreinbeheer/beheerregios/anb_beheerregios.shp")

map_Tsi <- ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = am_patdat, aes(fill = rchtnnb, color = rchtnnb), size = 0.5) +
  #geom_sf(data = df_Pl, aes(text = paste0(gebnaam, " (", gebied, "): ", round(overlap * 100, 1), "%")), fill = "#c04384", color = "#c04384", size = 0.5) +
  geom_sf(data = occs_Tsi, color = "blue", size = 1) +
  theme_map() +
  theme(
...
##### in PATDAT (excl ob)
```{r in-patdat, echo=FALSE, warning=FALSE}
in_PATDAT <- PATDAT_excl %>%
  dplyr::filter(gebied == "PATDAT_excl_ob" & type == "in")  %>%
  left_join(species_list, by = c("soort" = "Soort")) %>%
  select(soort, species, Groep, overlap) %>%
  distinct() %>%
  mutate(Groep = recode(Groep, "plant" = "planten", "dier" = "dieren"))
...
PATDAT_in_vs_of <- in_PATDAT %>%
  dplyr::filter(overlap != 0) %>%
  left_join(of_PATDAT, by = c("soort", "species"), suffix = c(".in", ".of")) #%>%
...
in_PATDAT <- PATDAT %>%
  dplyr::filter(gebied == "PATDAT_incl_ob" & type == "in") %>%
  transmute(soort, "in_patdat" = overlap)

of_PATDAT <- PATDAT %>%
  dplyr::filter(gebied == "PATDAT_incl_ob" & type == "of") %>%
  transmute(soort, "of_patdat" = overlap)

PATDAT_ngeb <- PATDAT %>%
  dplyr::filter(gebied == "PATDAT_incl_ob" & type == "n_domeinen") %>%
  transmute(soort, "n_domeinen" = overlap)

PATDAT_overview <- of_PATDAT %>%
  left_join(PATDAT_ngeb, by = "soort") %>%
  left_join(in_PATDAT, by = "soort") %>%
  mutate(
    of_patdat = case_when(
      of_patdat < 0.005 & of_patdat > 0 ~ "<0.01",
      TRUE ~ as.character(round(of_patdat, 2))  # Afronden op 2 decimalen
    ),
    in_patdat = case_when(
      in_patdat < 0.005 & in_patdat > 0 ~ "<0.01",
      TRUE ~ as.character(round(in_patdat, 2))  # Afronden op 2 decimalen
    ),
    n_domeinen = ifelse(is.na(n_domeinen), 0, n_domeinen)
  ) %>%
  rename(
    bezetting = of_patdat,
    `aantal ANB-domeinen` = n_domeinen,
    specificiteit = in_patdat
  ) %>%
  arrange(desc(specificiteit))

kable(PATDAT_overview)
```


#### opdeling voorkomen soorten volgens beheerrecht ANB
```{r}
data <- PATDAT %>%
  filter(type %in% c("anb_rechten")) %>%
  select(soort, gebied, overlap) 

soorten <- c("Siberische grondeekhoorn", "sikahert", "Chinese moerasslak", "kleine waterteunisbloem", "watercrassula")
...
group_order <- c("Eigendom", 
                 "Technisch beheer conform bosdecreet", 
                 "Beheerovereenkomst", 
                 "Protocol landsverdediging", 
                 "Huur", 
                 "Erfpacht")

soort_order <- c("Siberische grondeekhoorn", "sikahert", "Chinese moerasslak", "kleine waterteunisbloem", "watercrassula")

data_filtered$gebied <- factor(data_filtered$gebied, levels = rev(group_order))
data_filtered$soort <- factor(data_filtered$soort, levels = rev(soort_order))

color_mapping <- c(
  "Erfpacht" = "#bf812d",
  "Huur" = "#dfc27d",
  "Protocol landsverdediging" = "#f6e8c3",
  "Beheerovereenkomst" = "#c7eae5",
  "Technisch beheer conform bosdecreet" = "#80cdc1",
  "Eigendom" = "#35978f" 
)

ggplot(data_filtered, aes(x = overlap, y = soort, fill = gebied)) +
  geom_col(alpha = 0.90, width = 0.7) +
  #geom_vline(xintercept = 50, linetype = 2) +
  #scale_x_continuous(expand = expansion(mult = c(0,0.01)), labels = scales::percent_format(scale = 1)) +
  scale_fill_manual(values = color_mapping) +
  labs(x = "Aantal ANB-domeinen") +
  scale_x_continuous(
    expand = expansion(mult = c(0, 0.05)),
  ) +  
  custom_theme() +
...
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/patdat_ndom_per_recht_update.png", 
#        width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/patdat_ndom_per_recht_update.png", 
       width = 23, height = 10, units = "cm", dpi=300)
```


#### Waar liggen de verantwoordelijkheden per soort?
```{r responsibility-per-species-patdat, echo=FALSE, warning=FALSE}
data <- PATDAT %>%
  dplyr::filter(gebied != "PATDAT_incl_ob" & type == "in") %>%
  select(soort, gebied, overlap) 
...
data_filtered <- data %>%
  ...
  bind_rows(
    data %>%
      filter(soort %in% soorten) %>%
      group_by(soort) %>%
      summarise(buiten_patdat = 1 - sum(overlap)) %>%
      mutate(gebied = "Buiten ANB-patrimonium", overlap = buiten_patdat) %>%
      select(-buiten_patdat))

group_order <- c("Buiten ANB",
                 "Kust & Westhoek",
                 "Zandig Vlaanderen",
                 "Vlaamse Ardennen & Schelde-Leie",
                 "Groene Gordels",
...
color_mapping <- c(
  "Buiten ANB" = "grey60",
  "Kust & Westhoek" = "#f6e8c3",
  "Zandig Vlaanderen" = "#dfc27d",
  "Vlaamse Ardennen & Schelde-Leie" = "#bf812d",
  "Groene Gordels" = "#35978f",
  "Brabantse Wouden" = "#80cdc1",
  "Taxandria" = "#c7eae5",
  "Demerland & Zuiderkempen" = "#d98eb5",
  "Antwerpse Kempen" = "#c04384",
  "Lage tot aan Hoge Kempen" = "#f2d9e6",
  "Hoge Kempen tot Voeren"= "#e6b4ce")
...
ggsave("./radius/data/output/figuren/patdat_perc_per_br.png",
       width = 23, height = 13, units = "cm", dpi=300)
```
...
ggplot() +
  geom_text(data = x, aes(x = ofVL*100, y = ofPATDAT*100, label = abbr), size = 4) +
  geom_abline() +
  geom_abline(intercept = c(-2,2), linetype = 2) +
  coord_equal(ratio = 1) +
  labs(x = "% Vlaanderen", y = "% ANB-patrimonium") +
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/ofVl_vs_ofPATDAT_update.png",
       width = 23, height = 13, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/ofVl_vs_ofPATDATr_update.png",
       width = 23, height = 13, units = "cm", dpi=300)
```
...
x$gebied <- factor(x$gebied, levels = c(
                 "Kust & Westhoek",
                 "Zandig Vlaanderen",
                 "Vlaamse Ardennen & Schelde-Leie",
                 "Groene Gordels",
...
facet_labels <- c("Kust & Westhoek" = "Kust & \nWesthoek",
                 "Zandig Vlaanderen" = "Zandig \nVlaanderen",
                 "Vlaamse Ardennen & Schelde-Leie" = "Vlaamse \nArdennen & \nSchelde-Leie",
                 "Groene Gordels" = "Groene \nGordels",
                 "Brabantse Wouden" = "Brabantse \nWouden",
                 "Taxandria" = "Taxandria",
                 "Demerland & Zuiderkempen" = "Demerland & \nZuiderkempen",
                 "Antwerpse Kempen" = "Antwerpse \nKempen",
                 "Lage tot aan Hoge Kempen" = "Lage tot aan \nHoge Kempen",
                 "Hoge Kempen tot Voeren" = "Hoge Kempen \ntot Voeren")
...
PATDAT_slice$type <- factor(PATDAT_slice$type, levels = c("min", "max"))

## max 5
ggplot(data = PATDAT_max5, aes(x = factor + 0.0001, y = reorder_within(soort, factor, gebied), fill = gebied, color = gebied)) +
  geom_point(color = "#c04384", size = 1, show.legend = FALSE) +
  facet_wrap(~ gebied, scales = "free_y", ncol = 1) +
  scale_y_reordered() +
  coord_trans(x = "log2") +
  scale_x_continuous(
    breaks = c(0.0001, 1, 10, 100),
    labels = c("0", "1", "10", "100")
  ) +
  ...
  theme(plot.title.position = "plot",
        ...
        strip.text.x = element_text(size = 6, hjust = -0.01, vjust = 0),
        strip.text.y = element_text(size = 6, hjust = 0, vjust = 0))

ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/PATDAT_max5_per_br_update.png",  width = 3.1, height = 8, units = "in", dpi=300)
ggsave("./radius/data/output/figuren/patdat_max5_per_br_update.png",
       width = 3.1, height = 8, units = "in", dpi=300)

## min 5
ggplot(data = PATDAT_min5, aes(x = factor + 0.0001, y = reorder_within(soort, factor, gebied), fill = gebied, color = gebied)) +
  geom_point(color = "#c04384", size = 1, show.legend = FALSE) +
  facet_wrap(~ gebied, scales = "free_y", ncol = 1) +
  scale_y_reordered() +
  coord_trans(x = "log2") +
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/PATDAT_min5_per_br_update.png",  width = 3.1, height = 8, units = "in", dpi=300)
ggsave("./radius/data/output/figuren/patdat_min5_per_br_update.png",
       width = 3.1, height = 8, units = "in", dpi=300)

## min/max combined
# ggplot(data = PATDAT_slice, aes(x = factor + 0.0001, y = reorder_within(soort, factor, gebied), fill = gebied, color = gebied)) +
#   geom_point(color = "#c04384", size = 1.5, show.legend = FALSE) +
#   facet_grid(gebied ~ type, scales = "free", labeller = labeller(gebied = facet_labels, type = c("min" = "Laagste gebondenheid", "max" = "Hoogste gebondenheid"))) +
#   scale_y_reordered() +
#   coord_trans(x = "log2") +
#   labs(x = "Factor", y = NULL) +
#   custom_theme() +
...
#         strip.text.x = element_text(size = 8, hjust = 0, vjust = 0),
#         strip.text.y = element_text(size = 8, hjust = 0, vjust = 0))
# 
# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/PATDAT/PATDAT_min_max_per_br.png",
#        width = 23, height = 25, units = "cm", dpi=300)
# ggsave("./radius/data/output/figuren/patdat_min_max_per_br.png",
#        width = 23, height = 27, units = "cm", dpi=300)
```
...
## sbp beekvissen stromen
ggplot() +
  geom_sf(data = Provincies_grenzen, fill= "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = lu_sbp_pls, color = "lightblue3") +
  theme_map()

## sbp beekvissen waterclusters
library(colorspace)

ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = clusters_sbp, fill = "grey85", color = "grey40",linewidth = 0.15,  alpha = 0.9) +
  #geom_sf_label(data = clusters_sbp, aes(label = CLUSTERNR), size = 4, fill = "white", alpha = 0.7) +
  geom_sf(data = lu_sbp_pls, fill="royalblue", color ="royalblue", linewidth = 0.6) +
  scale_fill_discrete_qualitative(palette = "Set 2", nmax = 21) +
  theme_map() +
  theme(
    legend.position = "none",
    legend.title = element_blank()
...
ggsave("./radius/data/output/fig_maps/map_sbp_pls.png", width = 23, height = 8, units = "cm")

ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = clusters_sbp, fill="royalblue", color ="royalblue", linewidth = 0.1,linewidth = 0.1, alpha = 0.1) +
...
kable(clusters_sbp %>% st_drop_geometry() %>% select(CLUSTERNR, CLUSTERNAAM),
      , col.names = c("Nr.", "Watercluster naam"), 
      align = "llr",
      #caption = "Overzicht van de habitatrichtlijngebieden (SBZ-H) in Vlaanderen en hun oppervlakte (in ha)", 
      booktabs = TRUE) %>%
  kable_styling(bootstrap_options = "responsive",
...
sbp_pls_species <- c("Amerikaanse stierkikker", "Kesslergrondel", "Pontische stroomgrondel", "marmergrondel", "zwartbekgrondel", "Californische rivierkreeft", "geknobbelde Amerikaanse rivierkreeft", "gestreepte Amerikaanse rivierkreeft", "gevlekte Amerikaanse rivierkreeft", "rode Amerikaanse rivierkreeft", "Turkse rivierkreeft","Chinese wolhandkrab"
) 
```

### Top tien soorten met grootste bezetting per SBP
```{r}
data <- SBP_pls_extra %>%
  filter(type == "of_sbp")

ggplot(data, aes(x = overlap * 100, y = reorder(soort, overlap))) +
  facet_wrap(~gebied, scales = "free_y", ncol = 1) +
  geom_bar(stat = "identity", fill = "#c04384", width = 0.7) +
  scale_x_continuous(expand = expansion(mult = c(0,0.05))) +
  labs(x = "Percentage bezetting") +
  custom_theme() +
  theme(
    axis.title.y = element_blank()
  )

# ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/SBP/sbp_bezetting_update.png",
#        width = 23, height = 16, units = "cm", dpi=300)
# ggsave("./radius/data/output/figuren/sbp_bezetting_update.png",
#        width = 23, height = 16, units = "cm", dpi=300)

data <- SBP_pls_extra %>%
  filter(gebied == "clusters" & type == "of_clust")
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/SBP/sbp_bezetting2_update.png",
       width = 23, height = 10, units = "cm", dpi=300)
```


```{r kaartjes-bezetting-sbps}
# SBP beekprik
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = lu_sbp_pls %>% filter(soort == "Beekprik"), color = "lightblue3", linewidth = 1) +
  geom_sf(data = sbp_pls_intersects %>% filter(gebied == "SBP  Beekprik"), aes(color = soort, fill = soort), linewidth = 2.5) +
  scale_fill_discrete_qualitative(palette = "Set 2", nmax = 10) +
  scale_colour_discrete_qualitative(palette = "Set 2", nmax = 10) +
  theme_map() +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
...
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = lu_sbp_pls %>% filter(soort == "Rivierdonderpad"), color = "lightblue3", linewidth = 1) +
  geom_sf(data = sbp_pls_intersects %>% filter(gebied == "SBP  Rivierdonderpad"), aes(color = soort, fill = soort), linewidth = 2.5) +
  scale_fill_discrete_qualitative(palette = "Set 2", nmax = 10) +
  scale_colour_discrete_qualitative(palette = "Set 2", nmax = 10) +
  theme_map() +
  theme(
...
# SBP kleine modderkruiper
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = lu_sbp_pls %>% filter(soort == "Kleine Modderkruiper"), color = "lightblue3", linewidth = 1) +
  geom_sf(data = sbp_pls_intersects %>% filter(gebied == "SBP  Kleine Modderkruiper"), aes(color = soort, fill = soort), linewidth = 2.5) +
  scale_fill_discrete_qualitative(palette = "Set 2", nmax = 10) +
  scale_colour_discrete_qualitative(palette = "Set 2", nmax = 10) +
  # scale_color_manual(values = c("Actueel leefgebied" = "darkblue",
  #                               "Uitbreiding leefgebied" = "lightblue")) +
  theme_map() +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
...
### Waar zitten soorten voornamelijk (in welke cluster)?
```{r}
data <- SBP_pls_extra %>%
  filter(type == "in_clust" & gebied != "clusters") %>%
  select(soort, gebied, overlap) 

cluster_order <- c("Buiten SBP", "IJzer", "bovenlopen van de Vlaamse Ardennen", 
                   "Waasland", "Zwarte Beek (Benedenscheldebekken)", "Schijn", 
                   "Zennebekken ten zuiden van Brussel", "Dijlebekken ten zuiden van Leuven", 
                   "middenloop en benedenloop Kleine Nete", "Molenbeek-Bollaak", "Aa",
                   "bovenlopen Kleine Nete en Wamp", "middenloop en benedenloop Grote Nete",
                   "bovenlopen Grote Nete", "Zwarte Beek en Laambeek", 
                   "Kleine en Grote Gete", "Mombeek en Munsterbeek", 
                   "Mark (Maasbekken)", "Warmbeek en Dommel", 
                   "Abeek en Bosbeek", "Maas", "Voerstreek")

species_order <- c("Pontische stroomgrondel", "marmergrondel", "Kesslergrondel",
                   "Amerikaanse stierkikker", "gevlekte Amerikaanse rivierkreeft",
                   "Turkse rivierkreeft", "zwartbekgrondel",
                   "gestreepte Amerikaanse rivierkreeft", "Chinese wolhandkrab",
                   "rode Amerikaanse rivierkreeft", "Californische rivierkreeft", "geknobbelde Amerikaanse rivierkreeft")

data_filtered <- data %>%
  bind_rows(
    data %>%
      group_by(soort) %>%
      summarise(buiten_sbp = 1 - sum(overlap)) %>%
      mutate(gebied = "Buiten SBP", overlap = buiten_sbp) %>%
      select(-buiten_sbp)
  ) %>%
  mutate(
    gebied = factor(gebied, levels = cluster_order),
    soort = factor(soort, levels = rev(species_order))
  )

set2_colors <- qualitative_hcl(21, palette = "Set 2")
all_colors <- c("Buiten SBP" = "grey60", setNames(set2_colors, levels(data_filtered$gebied)[-1]))

ggplot(data_filtered, aes(x = overlap * 100, y = soort, fill = gebied)) +
  geom_col(alpha = 0.90, width = 0.7, color = "grey60") +
  geom_vline(xintercept = 50, linetype = 2) +
  scale_x_continuous(expand = expansion(mult = c(0,0.01)), labels = scales::percent_format(scale = 1)) +
...
```{r bezetting-clusters}
data <- SBP_pls_extra %>%
  filter(type == "of_clust" & gebied != "clusters" & overlap != 0) %>%
  group_by(gebied) %>%
  summarise(n_spec = n()) %>%
  add_row(gebied = "Kleine en Grote Gete", n_spec = 0) %>%
  left_join(clusters_sbp, by = c("gebied" = "CLUSTERNAAM"))
  

## barplot
ggplot(data, aes(x = n_spec, y = reorder(gebied, n_spec))) +
  geom_bar(stat = "identity", color = "#c04384", fill = "#c04384", width = 0.7) +
  labs(x = "# IUS") +
  custom_theme() +
  theme(
    axis.title.y = element_blank()
...
## kaartje
colors <- c("grey80","#f2d9e6", "#e6b4ce", "#d98eb5", "#c04384")
values <- c(0, 1, 3, 5, 7)  
...
ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", color = "black", linewidth = 0.2) +
  geom_sf(data = data$geom, aes(fill = data$n_spec), color = "grey20", linewidth = 0.15) +
  geom_sf(data = lu_sbp_pls, color = "royalblue", linewidth = 0.6) +
  geom_sf(data = occ_flanders %>% filter(Soort == "Chinese wolhandkrab"), color = "red") +
  labs(fill = "Aantal IUS") +
  scale_fill_gradientn(colors = colors, values = scales::rescale(values)) +
  theme_map() +
  theme(
    legend.background = element_blank(),
    legend.title = element_blank(),
...
leaflet() %>%
  addTiles(data = "OSM") %>%
  addPolygons(data = clusters_sbp %>% st_transform(4326), color = "grey") %>%
  addPolylines(data = lu_sbp_pls %>% st_transform(4326), fill = ~type) #%>%
  # addCircleMarkers(data = occ_flanders %>% filter(Soort == "Chinese wolhandkrab"), color = "red", radius = 1)

ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", color = "black", linewidth = 0.2) +
  geom_sf(data = data$geom, aes(fill = data$n_spec), color = "grey20", linewidth = 0.15) +
  geom_sf(data = lu_sbp_pls, color = "lightblue3", linewidth = 0.8) +
  labs(fill = "Aantal IUS") +
  scale_fill_gradientn(colors = colors, values = scales::rescale(values)) +
  theme_map() +
  theme(
...
ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/SBP/sbp_nspec_per_cluster_update.png",
       width = 23, height = 10, units = "cm", dpi=300)
ggsave("./radius/data/output/figuren/sbp_nspec_per_cluster_update.png",
       width = 23, height = 10, units = "cm", dpi=300)

ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", linewidth = 0.2) +
  geom_sf(data = lu_sbp_pls, color = "lightblue3", linewidth = 1) +
  geom_sf(data = sbp_pls_intersects, aes(color = soort, fill = soort), linewidth = 2.5) +
  scale_fill_discrete_qualitative(palette = "Set 2", nmax = 9) +
  scale_colour_discrete_qualitative(palette = "Set 2", nmax = 9) +
  theme_map() +
  theme(
    legend.position = "bottom",
    legend.title = element_blank(),
...
### Welke soorten per cluster? 
```{r}
data <- SBP_pls_extra %>%
  filter(type == "of_clust" & gebied != "clusters")

for (i in unique(data$gebied)) {
  print(ggplot(data %>% filter(gebied == i), aes(x = overlap, y = reorder(soort, overlap))) +
          geom_bar(stat = "identity", fill = "#c04384", width = 0.7) +
          labs(title = paste0(i)) +
          custom_theme())
}
...
```{r spec-in-nete}
cluster_nete <- clusters_sbp %>% filter(CLUSTERNAAM %in% c("middenloop en benedenloop Kleine Nete", "middenloop en benedenloop Grote Nete"))

bbox <- st_bbox(cluster_nete)

lu_sbp_pls_n <- lu_sbp_pls %>%
  st_intersection(cluster_nete)

sbp_pls_intersect_n <- sbp_pls_intersects %>%
  st_intersection(cluster_nete)

palet <- c(
  "Amerikaanse stierkikker" = "#ED90A4",
  "Chinese wolhandkrab" = "#DC9E70",
  "gestreepte Amerikaanse rivierkreeft" = "#B7AE50",
  "gevlekte Amerikaanse rivierkreeft" = "#7EBA68",
  "marmergrondel" = "#1DC199",
  "Pontische stroomgrondel" = "#00BFC8",
  "zwartbekgrondel" = "#6FB1E7"
)

ggplot() +
  geom_sf(data = Provincies_grenzen, fill = "#f0f0f0", color = "black", linewidth = 0.1) +
  geom_sf(data = cluster_nete) +
  geom_sf(data = lu_sbp_pls_n, color = "lightblue3", linewidth = 1) +
  geom_sf(data = sbp_pls_intersect_n, aes(color = soort, fill = soort), linewidth = 4, alpha = 1) +
  scale_fill_manual(values = palet, name = "Soort", drop = FALSE, na.translate = FALSE) +
  scale_color_manual(values = palet, name = "Soort", drop = FALSE, na.translate = FALSE) +
  guides(
  ...
  coord_sf(
    ...
    expand = FALSE, 
    crs = st_crs(cluster_nete)
  )

ggsave("./radius/data/output/figuren/sbp_nete_update.png",
       width = 23, height = 10, units = "cm", dpi=300)

ggsave("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/5_Rapport/Figuren & tabellen/Resultaten/SBP/sbp_nete_update.png",
       width = 23, height = 10, units = "cm", dpi=300)
