...
### tabel met opsplitsing volgens categorie
y <- N2KHAB %>%
  dplyr::filter(gebied != "N2KHAB" & type == "of" & overlap != 0) %>%
  left_join(species_list, by = c("soort" = "Soort"), relationship = "many-to-many") %>%
  left_join(habitats, by = c("gebied" = "code")) %>%
...
y <- y %>%
    mutate(bezetting_cat = case_when(
        n_spec == 0 ~ "0%", 
        max_overlap < 25 ~ "<25%",
        max_overlap >= 25 & max_overlap < 50 ~ "25-50%",
        max_overlap >= 50 & max_overlap < 75 ~ "50-75%",
        TRUE ~ ">75%"
    )) %>%
    mutate(bezetting_cat = factor(bezetting_cat, 
                                   levels = c("0%", "<25%", "25-50%", "50-75%", ">75%")))

y <- y %>%
    mutate(aantal_cat = case_when(
        n_spec == 0 ~ "0",  
        n_spec < 10 ~ "<10",
        n_spec >= 10 & n_spec < 20 ~ "10-20",
        n_spec >= 20 & n_spec < 30 ~ "20-30",
        TRUE ~ ">30"
    )) %>%
    mutate(aantal_cat = factor(aantal_cat, 
                                levels = c("0", "<10", "10-20", "20-30", ">30")))

result <- y %>%
    group_by(aantal_cat, bezetting_cat) %>%
    summarise(habitat = paste(gebied[!is.na(gebied)], collapse = ", "), .groups = 'drop') %>%
    pivot_wider(names_from = bezetting_cat, values_from = habitat) %>%
    arrange(aantal_cat) 
