...
ggsave(file=paste0("G:/.shortcut-targets-by-id/0B0xcP-eNvJ9dMGhEZjVoT28tVVk/PRJ_FAUNABEHEER/INBOPRJ-21105 - RadIUS/4_Workshop/Presentaties/Figuren/RadIUS/ofpatdat_ofvl.png"), width = 10, height = 13, units = "cm", dpi=200)
```
...
```{r}
for (i in unique(inPATDAT$Soort)) {
  df <- inPATDAT %>%
    filter(Soort == i)
  
  print(ggplot() +
          geom_point(data = df, aes(x = ofVL + 0.000001, y = inBR + 0.000001, color = regio)) +
          geom_abline() +
          ylab("inANB") +
          labs(title = i) +
