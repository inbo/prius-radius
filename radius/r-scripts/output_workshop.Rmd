...
par(family = "Arial")
data <- data.frame(
  category = c("niet van toepassing", "boswachter", "overig ANB-personeel", "regiobeheerder"),
  count = c(7, 11, 8, 2)
)
...
responses_pie <- ggplot(data, aes(x = "", y = count, fill = category)) +
  geom_bar(width = 1, stat = "identity", color = "grey98", linewidth = 0.5 ) +
  # geom_label(
  #   aes(label = paste0(count, " (", round((count / 28) * 100, 1), "%)")), 
  #   color = "black", 
  #   fill = "white",
  ...
  #   show.legend = FALSE
  # ) +
  #geom_text(aes(label = paste0(count, " (", round((count / 28) * 100, 1), "%)")), position = position_stack(vjust = 0.5), size = 3) +
...
```{r}
n_per_br <- data.frame(
  beheerregio = c("Antwerpse Kempen", "Brabantse Wouden", "Demerland & Zuiderkempen", "Groene Gordels", "Hoge Kempen tot Voeren", "Kust & Westhoek", "Lage tot aan Hoge Kempen", "Taxandria", "Vlaamse Ardennen & Schelde-Leie", "Zandig Vlaanderen", "Niet van toepassing"), 
  aantal = c(3,4,2,2,2,7,2,2,4,4,9)
) %>%
  mutate(perc = paste0(round((aantal / 28) * 100, 1), "%"),
         beheerregio = factor(beheerregio, levels = rev(c("Antwerpse Kempen", "Brabantse Wouden", "Demerland & Zuiderkempen", 
...
plot_n_per_br <- ggplot(n_per_br, aes(x = aantal, y = beheerregio)) +
  geom_bar(stat = "identity", fill = "#c04384", width = 0.8) +
  geom_text(aes(label = paste0(aantal, " (", perc, ")")), hjust = -0.1, size = 3) + 
...
png("G:/Mijn Drive/Fleur/RadIUS/RadIUS voorbereiding/workshop_attendance.png", 
    width = 25, height = 10, units = "cm", res = 300)


grid.arrange(right_column, plot_n_per_br, ncol = 2, widths = c(2,5))

# Add your text labels
# grid.text("A.", x = 0.02, y = 0.97, gp=gpar(fontsize=10, fontface = "bold"))
# grid.text("B.", x = 0.52, y = 0.65, gp=gpar(fontsize=10, fontface = "bold"))
