...
```{r prius_list}
prius_list <- read_csv(here("prius", "data", "input", "prius_species_list.csv"), locale = locale(encoding = "ISO-8859-1"), show_col_types = FALSE)  
```
...
union_list <- name_usage(datasetKey = datasetKey, limit = 10000)$data %>%
  subset(rank != "KINGDOM" & !is.na(nubKey)) #Animalia, Chromista, Plantae wegfilteren
```

! Opgelet: in versie van unielijst zoals beschikbaar op 12/02/2026 is er geen nubKey voor *Neogale vison* en *Triadica* spec. --> deze laat ik daarom achterwege
...
write_csv(radius_list_update, here("radius", "data", "input", "radius_species_list.csv"))
```
