...
5. Combining species lists
6. verifying GBIF keys against GBIF Backbone
7. Prepare RadIUS species list for export
...
ANB_list <- data.frame(matrix(ncol=5,nrow=5, dimnames=list(NULL, names(radius_list)))) %>%
  mutate(
    GBIF_code = c(2440954, 9738098, 8889878, 10919373, 8946295), 
    Soort = c("sikahert", "Chinese moerasslak", "Chinese prachtslang", "Mantsjoerese wilde rijst", "Turkse rivierkreeft"), 
    Lijst = "0")
...
```{r combining-lists}
radius_list_update <- rbind(radius_list, riparias_list, dunias_list, ANB_list) %>%
...
radius_list_update <- radius_list_update %>%
  transmute(Groep, Species, Soort, GBIF_scientific_name, GBIF_code, 
            ...
            ANB_lijst = ifelse(Lijst == "ANB" | GBIF_code %in% ANB_list$GBIF_code, 1, 0))
```
