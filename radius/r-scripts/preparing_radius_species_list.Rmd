...
Resultaat: 
  ...
  - 2 soorten: synoniemen aanwezig in Union-list: Amerikaanse stierkikker (Lithobates catesbeianus 2427091, Rana catesbeiana 2427092) & Fraai lampenpoetsersgras (Pennisetum setaceum 2706134, Cenchrus setaceus 5828232)

```{r adjusting-hydrocotyle-ranunculoides-GBIF-code}
prius_list$GBIF_code[prius_list$GBIF_code == 3034613] <- 7978544
```

# Checking GBIF keys
```{r prius_key_check}
prius_key_check <- gbif_verify_keys(prius_list, col_keys = "GBIF_code") %>%
  full_join(prius_list, by = c("key" = "GBIF_code"))
```

```{r export-prius-list}
write.csv2(prius_list, "./radius/data/input/radius_species_list.csv")
```
