...
4. Adding DUNIAS project species 
5. Adding crayfish
5. Checking GBIF codes 
...
riparias_species <- riparias_list[!riparias_list$nubKey %in% radius_list_update$GBIF_code,] %>%   #only select species that are not already present in the radius_list_update
  ...
  mutate(Species = if_else(Species == "Egeria densa", "Elodea densa", Species)) %>%
  mutate(GBIF_code = if_else(GBIF_code == 5329260,5329263, GBIF_code))
...
## al in PrIUS-lijst: hemelboom, struikaster
## nog niet in PrIUS-lijst: mahonie (3033868), boksdoorn (2928835), rimpelroos (3003979), smalle olijfwilg (3039269), sneeuwbes (8012439), vijfbladige wingerd (3039194), valse wingerd (3039203)

```{r adding-dunias-project species}
dunias_species <- data.frame(matrix(ncol=5,nrow=7, dimnames=list(NULL, c("Groep","Species","Soort","Lijst","GBIF_code")))) %>%
  mutate(GBIF_code = c("3033865","2928835","3003979","3039269","8012439","3039194","3039203")) %>%
  mutate(Soort = c("mahonie","boksdoorn","rimpelroos","smalle olijfwilg","sneeuwbes","vijfbladige wingerd","valse wingerd"))

for(i in 1:nrow(dunias_species)) {
  name_usage(key = dunias_species$GBIF_code[i]) -> temp
  temp$data$kingdom -> dunias_species$Groep[i]
  temp$data$scientificName -> dunias_species$Species[i]
}
rm(i,temp)

dunias_species <- dunias_species %>%
  mutate(Groep = ifelse(Groep == 'Plantae',"plant","dier"), 
         Lijst = "DUNIAS")
```

```{r combining-radius-dunias}
radius_list_update <- rbind(radius_list_update, dunias_species)
```

# 5. Adding crayfish 
```{r}
crayfish <- data.frame(matrix(ncol=5,nrow=1, dimnames=list(NULL, c("Groep","Species","Soort","Lijst","GBIF_code")))) %>%
  mutate(GBIF_code = c("8946295")) %>%
  mutate(Soort = c("Turkse rivierkreeft")) %>%
  mutate(Groep = "dier")

for(i in 1:nrow(crayfish)) {
  name_usage(key = crayfish$GBIF_code[i]) -> temp
  temp$data$scientificName -> crayfish$Species[i]
}
rm(i,temp)
...
```{r adding-crayfish}
radius_list_update <- rbind(radius_list_update, crayfish)
```


# 6. verifying GBIF keys against GBIF Backbone
