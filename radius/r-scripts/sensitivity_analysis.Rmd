...
```
```{r save-results}
sa_hbtrl <- list.results$sa_hbtrl
write.csv(sa_hbtrl,"./radius/data/intermediate/sensitiviteitsanalyse/sa_hbtrl.csv")

sa_vglrl <- list.results$sa_vglrl
write.csv(sa_vglrl,"./radius/data/intermediate/sensitiviteitsanalyse/sa_vglrl.csv")

sa_patdat_regio <- list.results$sa_patdat_regio
write.csv(sa_patdat_regio,"./radius/data/intermediate/sensitiviteitsanalyse/sa_patdat_regio.csv")

sa_patdat_recht <- list.results$sa_patdat_recht
write.csv(sa_patdat_recht,"./radius/data/intermediate/sensitiviteitsanalyse/sa_patdat_recht.csv")

sa_nbhp_eigdm <- list.results$sa_nbhp_eigdm
write.csv(sa_nbhp_eigdm,"./radius/data/intermediate/sensitiviteitsanalyse/sa_nbhp_eigdm.csv")

sa_nbhp_type <- list.results$sa_nbhp_type
write.csv(sa_nbhp_type,"./radius/data/intermediate/sensitiviteitsanalyse/sa_nbhp_type.csv")
```

```{r inlezen-data-voor-plotten}
sa_hbtrl <- read.csv("./radius/data/intermediate/sensitiviteitsanalyse/sa_hbtrl.csv")
sa_vglrl <- read.csv("./radius/data/intermediate/sensitiviteitsanalyse/sa_vglrl.csv")
sa_patdat_regio <- read.csv("./radius/data/intermediate/sensitiviteitsanalyse/sa_patdat_regio.csv")
sa_patdat_recht <- read.csv("./radius/data/intermediate/sensitiviteitsanalyse/sa_patdat_recht.csv")
sa_nbhp_eigdm <- read.csv("./radius/data/intermediate/sensitiviteitsanalyse/sa_nbhp_eigdm.csv")
sa_nbhp_type <- read.csv("./radius/data/intermediate/sensitiviteitsanalyse/sa_nbhp_type.csv")
```

```{r plot-hbtrl}
ggplot(data = sa_hbtrl, aes(x = Buffer, y = Overlap, group = Soort, color = Soort), show.legend = FALSE) +
  geom_point() +
  geom_line() +
  theme_minimal()

plot_ly(sa_hbtrl, x = ~Buffer, y = ~Overlap, type = 'scatter', mode = 'nodes + lines', color = ~Soort, showlegend = FALSE) %>%
  layout(title="HBTRL", xaxis = list(title = "Buffer afstand (m)"), yaxis = list(title = "Percentage in HBTRL") )
```

```{r plot-vglrl}
ggplot(data = sa_vglrl, aes(x = Buffer, y = Overlap, group = Soort, color = Soort), show.legend = FALSE) +
  geom_point() +
  geom_line() +
  theme_minimal()

plot_ly(sa_vglrl, x = ~Buffer, y = ~Overlap, type = 'scatter', mode = 'nodes + lines', color = ~Soort, showlegend = FALSE) %>%
  layout(title="VGLRL", xaxis = list(title = "Buffer afstand (m)"), yaxis = list(title = "Percentage in VGLRL") )
```

```{r plots-patdat}
ggplot(sa_patdat_regio, aes(x = Buffer, y = Overlap, fill = Gebied, group = Gebied)) + 
    geom_bar(position="fill", stat="identity") +
    facet_wrap(~Soort) + 
    theme(axis.text.x = element_text(angle = 90))

ggplot(sa_patdat_recht, aes(x = Buffer, y = Domeinen, fill = Recht, group = Recht)) + 
    geom_bar(position="stack", stat="identity") +
    facet_wrap(~Soort, scales = "free") + 
    theme(axis.text.x = element_text(angle = 90))

```

```{r plots-nbhp}
ggplot(sa_nbhp_eigdm, aes(x = Buffer, y = Domeinen,fill = Type,  group = Type)) + 
    geom_bar(position="stack", stat="identity") +
    facet_wrap(~Soort, scales = "free") + 
    theme(axis.text.x = element_text(angle = 90))

ggplot(sa_nbhp_type, aes(x = Buffer, y = Overlap, fill = Type,  group = Type)) + 
    geom_bar(position="fill", stat="identity") +
    facet_wrap(~Soort) + 
    theme(axis.text.x = element_text(angle = 90))
...
```{r model-hbtrl}
model_hbtrl <- lmer(Overlap ~ as.factor(Buffer) + (Buffer | Soort), family = "binomial", data = sa_hbtrl)
```