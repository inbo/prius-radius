---
title: "Calculating occupancy in protected areas"
...
for (soort in unique(occ_flanders$Soort)) {
  ...
  occ_temp <- occ_flanders[occ_flanders$Soort == soort,]
  disp_dist <- unique(occ_temp$disp_dist)
  
  # Creëer een buffer rond soortwaarnemingen op basis van dispersieafstand
  occ_temp_buf <- occ_temp %>%
  ...
  total_area <- sum(occ_temp_buf$area)
  ...
  for (i in 1:length(list.dfs)) {
    ...
    occ_temp_buf <- occ_temp_buf %>%
        st_transform(31370)
    
    # Intersection tussen waarnemingsgegevens en wfs-data
    occ_intersection <- occ_temp_buf %>%
      st_intersection(gebied) %>% 
      mutate(intersect_area = st_area(.)) %>%
      st_transform(31370)
    
    overlap_intersection <- sum(occ_intersection$intersection_oppervlakte)
    percentage_overlap = as.numeric(sum(occ_intersection$intersect_area)/total_area)
    ...
    # Bereken het overlappercentage per gebied 
    overlap_per_gebied <- occ_intersection %>%
    ...
    # Bij alle soorten gebieden (habitatrichtlijn, vogelrichtlijn, natuurbeheerplannen, ANB patrimoniumdatabank), behalve habitatkaart (i = 5), selecteren we alleen de top 3 gebieden
    if (i != 5) {
    ...
    if (i == 4) {
      for (j in c(6,7)) {
        overlap_patdat <- occ_intersection %>%
          group_by(across(j)) %>%
          summarise(intersect_area = sum(intersect_area)) %>%
          arrange() %>%
          st_drop_geometry()
    ...
    occ_intersection <- occ_intersection %>%
        st_transform(4326)
    ...
    #        addPolygons(data = occ_temp_buf, color = "blue", label = ~area) %>%
    #        addPolygons(data = occ_intersection, color = "red", label = ~intersect_area) %>%
    #        addCircleMarkers(data = occ_temp, color = "black", radius = 1, fillOpacity = 1, fillColor = "black") %>%
