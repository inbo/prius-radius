...
for (soort in unique(occ_flanders$Soort)) {
  ...
  for (i in 1:length(list.dfs)) {
    ...
    if (i == 4) {
      ...
      # percentage overlap met anb domeinen
      list.results[[i]] <- rbind(list.results[[i]], data.frame(
        Soort = soort,
        Gebied = paste0("in", names(list.dfs)[i]),
    ...
    if (i == 5) {
      ...
      list.results[[i]] <- rbind(list.results[[i]], data.frame(
          Soort = soort,
          Gebied = "nNBHP",
          Value = length(unique(occ_species_intersection$id))))

      # aantal bezette domeinen per type natuurbeheerplan
      nbhp_type <- occ_species_intersection %>%
        group_by(id) %>%
        st_drop_geometry() %>%
        mutate(buf_area = sum(buf_area),
              intersect_area = sum(intersect_area)) %>%
        unique()
        
        
      n_per_type <- nbhp_type %>%
        group_by(natuurbeheerplantype_abb) %>%
        summarise(n = n()) 
      
      for (row in 1:nrow(n_per_type)) {
        list.results[[i]] <- rbind(list.results[[i]], data.frame(
          Soort = soort,
          Gebied = paste0("n", as.character(n_per_type[row,1])),
          Value = as.character(n_per_type[row,2])))
      }
      
      # percentage overlap met natuurbeheerplannen
      list.results[[i]] <- rbind(list.results[[i]], data.frame(
        Soort = soort,
        Gebied = paste0("in", names(list.dfs)[i]),
        Value = format(round(percentage_overlap_in, 3))))
