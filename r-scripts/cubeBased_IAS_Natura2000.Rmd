...
for (species_name in unique(in_cube$Soort)){
  ...
  N_occ_spec_in_Natura2000 <- as.numeric(n_distinct( IAS_Cube_temp[which(IAS_Cube_temp$eea_cell_code%in%Natura2000$CELLCODE),]))
  N_occ_spec_in_spa <- as.numeric(n_distinct( IAS_Cube_temp[which(IAS_Cube_temp$eea_cell_code%in%spa$CELLCODE),]))
  N_occ_spec_in_habitat <- as.numeric(n_distinct( IAS_Cube_temp[which(IAS_Cube_temp$eea_cell_code%in%habitat$CELLCODE),]))
